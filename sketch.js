...
const SPEECH_DURATION = 3500; // <--- Duração da fala em milissegundos (agora 35 segundos)
// ---------------------------------------------
...
function keyPressed() {
  if (key === 'n' || key === 'N') {
    ...
    resetSceneElements(); // Chama a função de reset para o novo cenário
  }
}
...
function resetSceneElements() {
  ...
  if (currentScene === 0) { // Se for para o campo
    truckX = -200;
    showTruck = false;
    startTime = millis();
    ...
    showSecondCharacterSpeech = false;
    showSecondCharacterSpeech2 = false;
    // Resetar as falas da feira (garante que não iniciem antes da hora)
    showGreenSpeechFair1 = false;
    greenSpeechFairTime1 = 0;
    showGreenSpeechFair2 = false;
    greenSpeechFairTime2 = 0;
    ...
    // Os bonecos JÁ foram escondidos na lógica de transição no draw()
    // Apenas garante que não apareçam por padrão aqui.
    showCharacter = false;
    showSecondCharacter = false;
    showSacks = false; // Não há sacolas no cenário final
    characterState = 5; // Estado estático para a cena final

    // As posições dos personagens não importam, pois eles não serão desenhados
    // a menos que você mude showCharacter/showSecondCharacter para true aqui.
    characterX = width * 0.4;
    characterY = height * 0.7 - 60;
    secondCharacterX = width * 0.6;
    secondCharacterY = height * 0.7 - 60;

    // Garante que nenhuma fala esteja ativa na cena final
    showCharacterSpeech = false;
    showSecondCharacterSpeech = false;
    showSecondCharacterSpeech2 = false;
    showGreenSpeechFair1 = false;
    ...
    showTruck = false;
    riceSeeds = [];
    finalSceneTransitionTime = 0; // Zera para o próximo ciclo
  }
}